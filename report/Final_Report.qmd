...
plt.show()
```
The correlation matrix between numerical variables and playtime shows that price, peak concurrent users, and positive review rates all have a positive correlation with median playtime of games, but very weak.
...
plt.title("Average Median Playtime by Release Year")
...
cat_cols = ['publishers', 'genres', 'estimated_owners']
...
for col in cat_cols:
    ...
    plt.ylim(0, 10000)
    plt.title(f"Median Playtime by Top {col} Categories")
    plt.xlabel(col)
...
#| message: false
#| warning: false
>>>>>>> 0055a315a79be94d12a8bd511c3854a7855b4425
